...
electron_dir="$repo_dir/app/electron"

require_command npm
require_command node

app_name="$(cd "$electron_dir" && node -p "require('./package.json').name")"
...
printf 'Bundling Electron app...\n'
(cd "$repo_dir" && npm run bundle --workspace=app/electron)

shopt -s nullglob
artifacts=("$electron_dir"/bundle/"$app_name"-*-win32-"$target_arch".zip)
shopt -u nullglob

if [[ "${#artifacts[@]}" -eq 0 ]]; then
    printf 'No bundle artifact found for architecture %s in %s\n' "$target_arch" "$electron_dir/bundle" >&2
    exit 1
fi

artifact_file="$(printf '%s\n' "${artifacts[@]}" | sort -V | tail -n 1)"
run_dir="$electron_dir/.local-run/current"

printf 'Installing %s...\n' "$(basename "$artifact_file")"
install_bundle "$artifact_file" "$run_dir"

printf 'Done. Launch with launch-local-electron.cmd\n'
